 ...
 */
@Slf4j
public class ClusterExchangeStub
{
    ...
    private StreamObserver<ClusterPayloadProto.Response> responseStream;
    private final Map<UUID, CompletableFuture<Response>> responseFutures = new ConcurrentHashMap<>();

    final AtomicBoolean isClusterConnected = new AtomicBoolean(false);

    public ClusterExchangeStub()
    {
        responseStream = new StreamObserver<>()
        {
    ...
    public CompletableFuture<Response> request(Request request)
    {
        ...
        responseFuture.completeOnTimeout(
            new ResponseTimeout().status(408).code(408),
            timeoutMilliseconds,
